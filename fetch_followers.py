...
for name in json['accounts']:
    ...
    u = api.GetFollowers(screen_name=name, skip_status=True, count=200, include_user_entities=False)
    for item in u:
        # userobj=item
        # print dir(userobj)
        friends.append(item.screen_name)
    # userIdList = api.GetFollowerIDs(screen_name=name)
    # for userid in userIdList:
    #     u=api.GetUser(userid)
